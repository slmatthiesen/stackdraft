...
SECURITY FLOOR — STATED ONCE, NOT PER TIER (safe-by-default is non-negotiable). The full security floor is identical on every tier and it never moves, so emit it EXACTLY ONCE in the top-level securityFloor array: one short line per baseline below, covering all of them. Do NOT repeat the floor inside any tier — no per-tier security prose. Apply node-specific controls as short security TAGS on the relevant nodes (e.g. a private datastore tagged "private subnet" + "KMS at rest"; an S3 node tagged "block public access"). The budget tier keeps the ENTIRE floor — "budget" is the MINIMUM SAFE COST, never "cheap because insecure".
...
NAT / EGRESS COST: when a tier places data stores in private subnets (baseline 'no-public-data-tier'), tag those nodes "private subnet" and note the recurring NAT-gateway processing + internet egress cost in that tier's delta. The secure private-subnet default is NOT free and must never be presented as if it were (the cost line is filled deterministically downstream).

OBSERVABILITY (first-class, every tier — expressed as structure, not prose): include centralized structured logging (CloudWatch Logs with retention), metrics + CloudWatch alarms on the golden signals (latency, error rate, saturation/throttles), and tracing (X-Ray / OpenTelemetry) across service boundaries. Represent the telemetry flow in the graph (service → CloudWatch Logs/metrics → alarm) with payload-labeled edges and observability tags on nodes; scale it up the tiers via delta (budget = logs + key alarms; balanced = + dashboards/tracing on critical paths; resilient = + aggregation, anomaly detection, SLO alarms). This OPERATIONAL observability is distinct from the CloudTrail/access-logging SECURITY baseline (audit).
...
WELL-ARCHITECTED & DECISIONS (be opinionated): frame the design through the six AWS Well-Architected pillars — operational excellence, security, reliability, performance efficiency, cost optimization, sustainability. Populate keyDecisions with the handful of LOAD-BEARING choices: decision, the option chosen, the real alternativesConsidered, and a one-line rationale naming the pillar trade-off and why alternatives were rejected. Then COMMIT: set recommendedTier to the one tier you would actually ship for THIS workload (using the intake answers), and justify it in recommendationRationale — tie it to the stated/assumed traffic, availability target, and compliance posture. Do not hedge across all three; recommend one.
...
OUTPUT: assumptions, clarificationsUsed, a single securityFloor (the floor stated once, one line per baseline), exactly three tiers, plus recommendedTier with recommendationRationale and the load-bearing keyDecisions (chosen vs alternatives + why). Each tier has nodes (service + ≤4-word role + short security tags, NO prose), payload-labeled edges, a delta (robustness vs the other tiers; budget states the baseline), costDrivers in each service's native cost unit, and tradeoffs versus the other two tiers.`;
