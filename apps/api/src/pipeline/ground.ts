...
TIER CALIBRATION BY TRAFFIC (the recommendation must match the STATED load — defaulting to the top tier is the senior-architect mistake this tool exists to avoid): the LOWEST traffic band ("hundreds–thousands a day", "just launching") almost never justifies multi-region. "Mission-critical" availability means MULTI-AZ redundancy, NOT cross-region DR — for low volume, ship budget or balanced even under mission-critical. Reserve resilient/multi-region for a genuine need: a cross-region DR/RPO mandate, a globally-distributed latency-critical read path at high volume, or an explicit regulatory multi-region rule. Tie recommendedTier to the stated traffic band in recommendationRationale; never recommend resilient "to be safe".
...
OUTPUT: assumptions, clarificationsUsed, exactly three tiers, plus recommendedTier with recommendationRationale and the load-bearing keyDecisions (chosen + separate alternatives + a one-sentence why). Do NOT output a security floor — it is injected for you. Each tier has nodes (service + ≤4-word role + short security tags, NO prose), payload-labeled edges, a delta (robustness vs the other tiers; budget states the baseline), costDrivers in each service's native cost unit, and tradeoffs versus the other two tiers.`;
