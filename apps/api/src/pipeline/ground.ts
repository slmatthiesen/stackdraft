...
NAT/VPC IS A TIER STEP-UP, NOT A FLOOR (cost honesty — do not bury the most common surprise in budget): the BUDGET tier should default to the no-VPC, no-NAT shape — serverless compute (Lambda) + managed/serverless stores (DynamoDB, S3, Aurora Serverless) reached over IAM, so there is NO always-on NAT gateway or ALB and the idle cost floor is ~$0. Introduce a VPC with private subnets + a NAT gateway (and an ALB) only when the design actually needs a VPC-bound service (RDS/Aurora-provisioned, ElastiCache, EC2, Fargate/ECS) — and when you do, that is a robustness/feature STEP-UP that belongs at BALANCED (or resilient), called out explicitly in that tier's delta AND as a keyDecision ("private subnet data tier → NAT gateway +~$33/mo always-on"). Never put a NAT gateway / ALB / VPC-bound store in the budget tier if a serverless-no-VPC shape satisfies the workload; the budget tier is the true low-cost floor, not a private-subnet stack with the NAT cost hidden in it.

STATEFUL / CAN'T-GO-SERVERLESS BUDGET SHAPE (single public instance, no NAT, no ALB): some workloads genuinely need an always-on box — a single-writer file DB (SQLite/DuckDB on a disk), a long-running stateful process, a legacy container. Serverless does not fit, so the rules above don't give a $0 floor. For THOSE, the honest budget tier is a SINGLE public-subnet instance: one EC2 (or ECS-on-EC2) in a PUBLIC subnet with a public IP/Elastic IP behind a restrictive security group, outbound internet DIRECT (no NAT gateway), and NO load balancer (the instance serves directly, or sits behind an existing CDN/Cloudflare). That is ~$5–15/mo, not ~$50+. Do NOT add a NAT gateway (a public-subnet instance needs none — NAT exists only to give PRIVATE-subnet resources outbound egress) and do NOT add an ALB for a single instance (an ALB load-balances MULTIPLE targets; one box doesn't need one). Reserve the private-subnet + NAT + ALB topology for BALANCED/RESILIENT where multi-AZ/horizontal scaling actually requires it, and state the budget tier's tradeoff explicitly (single-AZ, single public instance, direct egress) in its delta + a keyDecision. The public-subnet-with-tight-SG choice is a documented budget tradeoff, NOT a security-floor violation.
