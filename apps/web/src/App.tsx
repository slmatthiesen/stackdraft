...
function Home(): JSX.Element {
  ...
  // Lazy per-tier (fix A): the tier currently being generated by "+ Add tier", or null.
  const [addingTier, setAddingTier] = useState<TierName | null>(null);
  // Streaming progress (fix D): the live pipeline phase, output heartbeat, and the
  // design elements as they complete, fed to LoadingDraft so the wait shows the build.
  const [loadingPhase, setLoadingPhase] = useState<string>("");
  const [loadingChars, setLoadingChars] = useState<number>(0);
  const [loadingItems, setLoadingItems] = useState<StreamItem[]>([]);
  ...
  const startGeneration = async (
    ...
    applyOutcome(
      await generateStream(
        { description, answers, round, freshOnly },
        {
          onPhase: setLoadingPhase,
          onToken: setLoadingChars,
          onItem: (it) => setLoadingItems((prev) => [...prev, it]),
        },
      ),
      description,
    );
  ...
  const handleAnswers = async (answers: string[]): Promise<void> => {
    ...
    applyOutcome(
      await generateStream(
        { description: goal, answers, round },
        {
          onPhase: setLoadingPhase,
          onToken: setLoadingChars,
          onItem: (it) => setLoadingItems((prev) => [...prev, it]),
  ...
  // "+ Add tier" (fix A): generate balanced/resilient on demand from the budget-first
  // result. Sends the budget tier as the baseline; the server returns just the new
  // costed tier, which we splice in (ordered) and switch to. Stays on the current tier
  // until it lands, so the view never blanks.
  const handleAddTier = async (name: TierName): Promise<void> => {
    if (!result || addingTier) return;
    const budgetTier = result.tiers.find((t) => t.name === "budget");
    if (!budgetTier) return;
    setAddingTier(name);
    const outcome = await addTier({
      description: goal,
    ...
    setAddingTier(null);
    if (outcome.kind !== "tier") return; // leave the +Add button in place to retry
    setResult((prev) => {
      if (!prev) return prev;
      const tiers = [...prev.tiers.filter((t) => t.name !== name), outcome.tier].sort(
        (a, b) => TIER_NAMES.indexOf(a.name) - TIER_NAMES.indexOf(b.name),
      );
      return { ...prev, tiers };
    });
  ...
  return (
    <main className={`app ${submitted ? "app--submitted" : ""}`}>
      ...
      {phase === "loading" && (
        <LoadingDraft phase={loadingPhase} chars={loadingChars} items={loadingItems} />
      )}
      ...
      {phase === "result" && result && (
        <>
          ...
          <DesignResult
            ...
            onAddTier={(name) => void handleAddTier(name)}
